...
def argon2Hash(string):
    ...
    ph = PasswordHasher(time_cost=8, memory_cost=102400, parallelism=8, hash_len = 18, salt_len=16, encoding='utf-8', type= argon2.Type.ID)
...
def login():
    ...
    while True:
        ...
        try:
            ...
            usrChoice = py.confirm(text='Could not verify password', title='Access denied', buttons=['Try again','Exit'])    
...
def addPassword():
    ...
    keyword = py.prompt(title='Input keyword', text='Input the keyword for the password. This is what you will use to find the password in the database. This will usually be the website or company the password is for')
    ...
    #Hashes enKeyword
    enKeyword = hash(keyword) #NOTE: Fernet will generate a different string each time, even if the given string and key are the same. This means keywords need to be hashed instead of encrypted
    ...
    while enKeyword in db:
        ...
        keyword = py.prompt(title='Invalid keyword', text='That keyword is already in the database. Please shoose a different one')
    ...
    while True:
        ...
        pass2 = py.password(title='Confirm password', text='Please confirm your password for "' + keyword + '"', mask='*')
        ...
        if pass1 == pass2:
            ...
            exitCode = py.confirm(title='Passwords do not match', text='Passwords do not match', buttons=['Try again', 'Cancel'])
...
def accessPassword():
    ...
    while True:
        ...
        keyword = py.prompt(title='Input keyword', text='Input the keyword for the password you would like to access. This is usually the name of the website')
        ...
        if enKeyword not in db:
            response = py.confirm(title='Keyword does not exist', text='Keyword does not exist in the database', buttons=['Try again', 'Cancel'])
    ...
    usrChoice = py.confirm(title='Confirm', text='Your password will be shown on screen. Make sure nobody else can see your screen and that no screen recording software is active', buttons=['Continue', 'Cancel'])
...
def changePassword():
    ...
    while True:
        ...
        if enKeyword not in db:
            response = py.confirm(title='Keyword does not exist', text='Keyword does not exist in the database', buttons=['Try a different one', 'Cancel'])
    ...
    while True:
        ...
        if pass1 != pass2:
            usrInput = py.confirm(title='Passwords do not match', text='Passwords do not match', buttons=['Try again', 'Cancel'])
    ...
    #Gets confirmation
    confirmation = py.confirm(title='Are you sure?', text='Are you sure you want to change this password? The old password will be lost forever', buttons=['Yes, change password', 'No, keep old password'])
...
def removePassword():
    ...
    #Gets confirmation
    answer = py.confirm(title='Confirm', text='Are you sure you would like to delete your password for ' + keyword, buttons=['Delete it','Cancel'])
...
def verifyDataBase():
    ...
    if not os.path.exists(directory+"master.peapass"):
        choice = py.confirm(title='Set up database', text = 'It looks like a database hasn\'t been set up on this computer yet. Would you like to do so now?', buttons=['Yes','Not now'])
        ...
        while True:
            pass1 = py.password(title='Input password', text='Please input your master password. This is the password you will use to access all other passwords, so make sure it is a long & strong password', mask='*')
            ...
            if pass1 == None or pass2 == None:
                ...
                exitCode = py.confirm(title='Passwords do not match', text='Passwords do not match', buttons=['Try again','Exit'])
...
def deleteDatabase():
    ...
    confirmation = py.confirm(text='Are you sure you want to delete the entire database?',title='PeaPass', buttons=[dontDeleteStr, deleteStr, cancelStr])
    ...
    #Confirms yet again
    confirmation = py.confirm(text='Are you sure you want to delete the entire database? This will delete ALL of your passwords and you will not be able to recover them.',title='PeaPass', buttons=[dontDeleteStr, deleteStr, cancelStr])
    ...
    #Does a final check
    choice = py.confirm(text='All passwords will now be deleted pernamently', title='PeaPass', buttons=['Just do it already', 'Cancel'])
...
def databaseOptions():
    ...
    #Gets mode from user
    mode = py.confirm(text='What would you like to do?', title='PeaPass', buttons=['Delete database','Export database','Cancel'])
...
def GUI():
    ...
    while True:
        ...
        mode = py.confirm(title='Access Granted', text='Access granted. What would you like to do?', buttons=['Log out', 'Add password','Access password', 'Remove password', 'Change a password', 'Database options'])
